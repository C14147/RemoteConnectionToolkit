...
def GetElement(uiName, elementName):
    if uiName in G_UIElementAlias:
        if (
            uiName in G_UIElementAlias.keys()
            and elementName in G_UIElementAlias[uiName].keys()
        ):
...
def AddUserData(uiName, elementName, dataName, datatype, datavalue, isMapToText):
    ...
    G_UIElementUserDataArray[uiName][elementName].append(
        [dataName, datatype, datavalue, isMapToText]
    )
...
def SetImage(uiName, elementName, imagePath):
    ...
    if elementName.find("Label_") == 0 or elementName.find("Button_") == 0:
        ...
        if Control != None:
                    ...
                    for EBData in G_UIElementUserDataArray[uiName][elementName]:
                        if EBData[0] == "image":
                            ...
                            image_Resize = image.resize(
                                (Control.winfo_width(), Control.winfo_height()),
                                Image.ANTIALIAS,
                            )
            ...
            image = Image.open(imagePath).convert("RGBA")
            image_Resize = image.resize(
                (Control.winfo_width(), Control.winfo_height()), Image.ANTIALIAS
            )
...
def CenterDlg(uiName, popupDlg, dw=0, dh=0):
    ...
    if root != None:
        ...
        sy = root.winfo_y()
        popupDlg.geometry(
            "%dx%d+%d+%d" % (dw, dh, sx + (sw - dw) / 2, sy + (sh - dh) / 2)
        )
    else:
        import ctypes
...
def SetRoundedRectangle(control, WidthEllipse=20, HeightEllipse=20):
    if control != None:
        control.after(
            10, lambda: ShowRoundedRectangle(control, WidthEllipse, HeightEllipse)
        )
...
def ShowRoundedRectangle(control, WidthEllipse, HeightEllipse):
    ...
    HRGN = win32gui.CreateRoundRectRgn(
        0, 0, control.winfo_width(), control.winfo_height(), WidthEllipse, HeightEllipse
    )
            ...
            if "__pycache__" not in fileName:
                if ".git" not in fileName:
                    ...
                    if os.path.isdir(newPath):
                        ...
                        if alldirs == True:
                            ResultFilesArray.extend(
                                WalkAllResFiles(newPath, alldirs, extName)
                            )
                    else:
                        if extName == None:
                            ResultFilesArray.append(newPath)
...
class WindowDraggable:
    ...
    def Motion(self, event):
        if self.widget == event.widget or event.widget.winfo_class() == "Canvas":
            ...
            if (
                (x >= 0)
                and (x <= self.bordersize)
                and (y >= 0)
                and (y <= self.bordersize)
            ):
            ...
            if (y >= (formh - self.bordersize)) and (y <= formh):
                ...
                self.bottom_drag.place(
                    x=0,
                    y=(formh - self.bordersize),
                    width=formw,
            ...
            if (x >= (formw - self.bordersize)) and (x <= formw):
                ...
                self.right_drag.place(
                    x=(formw - self.bordersize),
                    y=0,
    ...
    def MoveDragPos(self, event):
        if self.widget == event.widget or event.widget.winfo_class() == "Canvas":
            ...
            geoinfo = str(
                "%dx%d+%d+%d"
                % (self.widget.winfo_width(), self.widget.winfo_height(), newX, newY)
            )
    ...
    def MoveDragSize_H1(self, event):
        ...
        geoinfo = str(
            "%dx%d+%d+%d"
            % (newW, self.widget.winfo_height(), formx, self.widget.winfo_y())
        )
        self.widget.geometry(geoinfo)
        self.left_drag.place(
            x=0, y=0, width=self.bordersize, height=self.widget.winfo_height()
        )
        self.x = event.x_root
    ...
    def MoveDragSize_H2(self, event):
        ...
        self.widget.geometry(geoinfo)
        self.right_drag.place(
            x=newW - self.bordersize, y=0, width=self.bordersize, height=formh
        )
        self.x = event.x_root
        self.widget.configure(cursor="plus")
    ...
    def MoveDragSize_V1(self, event):
        ...
        geoinfo = str(
            "%dx%d+%d+%d"
            % (self.widget.winfo_width(), newH, self.widget.winfo_x(), formy)
        )
        self.widget.geometry(geoinfo)
        self.top_drag.place(
            x=0, y=0, width=self.widget.winfo_width(), height=self.bordersize
        )
        self.y = event.y_root
        self.widget.configure(cursor="plus")
        ...
        if event.widget == self.left_drag:
            ...
            if y >= (formh - self.bordersize) and y <= formh:
                if self.bottom_drag == None:
                    self.bottom_drag = tkinter.Label(self.widget)
                self.bottom_drag.place(
                    x=0, y=formh - self.bordersize, width=formw, height=self.bordersize
                )
                self.bottom_drag.bind("<ButtonPress-1>", self.StartDrag)
                self.bottom_drag.bind("<ButtonRelease-1>", self.StopDrag)
        ...
        if event.widget == self.top_drag:
            ...
            if x >= (formw - self.bordersize) and x <= formw:
                ...
                if self.right_drag == None:
                    self.right_drag = tkinter.Label(self.widget)
                self.right_drag.place(
                    x=formw - self.bordersize, y=0, width=self.bordersize, height=formh
                )
                self.right_drag.bind("<ButtonPress-1>", self.StartDrag)
                self.right_drag.bind("<ButtonRelease-1>", self.StopDrag)
                self.right_drag.bind("<B1-Motion>", self.MoveDragSize_TR)
    ...
    while True:
        ...
        if text.find("style.configure(") >= 0:
            ...
            if fontindex_begin >= 0:
                ...
                StyleArray[stylename]["font"] = tkinter.font.Font(
                    family=fontsplitarray[0].replace('"', "").strip(),
                    size=int(fontsplitarray[1].replace('"', "").strip()),
                    weight=fontsplitarray[2].replace('"', "").strip(),
                )
                othertext = (
                    sytleValueText[0:fontindex_begin]
                    + sytleValueText[fontindex_end + 1 : -1]
                )
            else:
